    ...
    import { elements } from "@/components/background-elements";
                ...
                <AnimatePresence mode="wait">
                    <motion.div key={pathname}>
                        {elements.map((elem, index) => (
                            <motion.div key={index} className="absolute" style={elem.style} variants={variants} initial="initial" animate="animate" exit="exit" transition={elem.transition}>
                                <motion.div className={`w-full h-full`} initial={{ backgroundImage: gradienBackground }} animate={{ backgroundImage: gradienBackground }} style={{ x: moveX, y: moveY, transition: `all ${elem.transition.duration}s linear`, rotate: -40 }}/>
                            </motion.div>
                        ))}
                    </motion.div>
                </AnimatePresence>
